...
export async function POST(request: Request) {
  try {
    const { messages, service, attachmentsContext } = await request.json();
    ...
    if (userMessageCount >= 2) {
      try {
        ...
        // piso). No sabemos de antemano si es ruido puntual o un "sin match"
        // real, así que se reintenta una vez antes de resignarse — si el
        // catálogo de verdad no tiene nada parecido, el reintento tampoco
        // encuentra nada y sigue costando solo 1 llamada extra, no un loop.
        if (!matchResult) {
